...
///验证码倒计时（单位：秒）
const int kVerifyCodeCountDownSeconds = 300;
///支付倒计时
    ...
    dispatch_source_set_event_handler(_timer, ^{
        if(timeout<=0){ //倒计时结束，关闭
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                //设置界面的按钮显示 根据自己需求设置
                tipLabel.text = @"重新发送验证码";
                tipLabel.textColor = [UIColor YQBlueColor];
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                //设置界面的按钮显示 根据自己需求设置
                tipLabel.text = [NSString stringWithFormat:@"重新发送验证码(%@s)",strTime];
                tipLabel.textColor = [UIColor YQsubTitleColor136];
